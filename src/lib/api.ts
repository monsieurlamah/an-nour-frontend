...
export const storesApi = {
  list: (p?: ListParams) => apiRequest<StoreRead[]>(`/stores${qs({ skip: 0, limit: 100, ...p })}`),

  get: (id: number) => apiRequest<StoreRead>(`/stores/${id}`),

  create: (body: StoreCreate) => apiRequest<StoreRead>("/stores", { method: "POST", body }),

  update: (id: number, body: StoreUpdate) =>
    apiRequest<StoreRead>(`/stores/${id}`, { method: "PATCH", body }),

  delete: (id: number) => apiRequest<void>(`/stores/${id}`, { method: "DELETE" }),

  getUsers: (storeId: number) => apiRequest<StoreUserRead[]>(`/stores/${storeId}/users`),
  ...
  removeUser: (linkId: number) => apiRequest<void>(`/stores/users/${linkId}`, { method: "DELETE" }),
};
...
export const catalogApi = {
  ...
  getProduct: (id: number) => apiRequest<ProductRead>(`/catalog/products/${id}`),

  getProductByUuid: (uuid: string) => apiRequest<ProductRead>(`/catalog/products/by-uuid/${uuid}`),
  ...
  deleteProduct: (id: number) => apiRequest<void>(`/catalog/products/${id}`, { method: "DELETE" }),
...
export const clientsApi = {
  ...
  get: (id: number) => apiRequest<ClientRead>(`/clients/${id}`),

  create: (body: ClientCreate) => apiRequest<ClientRead>("/clients", { method: "POST", body }),

  update: (id: number, body: ClientUpdate) =>
    apiRequest<ClientRead>(`/clients/${id}`, { method: "PATCH", body }),

  delete: (id: number) => apiRequest<void>(`/clients/${id}`, { method: "DELETE" }),
};
...
export const commandesApi = {
  ...
  get: (id: number) => apiRequest<CommandeRead>(`/commandes/${id}`),

  listEvents: (id: number) => apiRequest<CommandeEvenementRead[]>(`/commandes/${id}/events`),

  listReceptions: (id: number) =>
    apiRequest<CommandeReceptionRead[]>(`/commandes/${id}/receptions`),

  listAnomalies: (id: number) => apiRequest<CommandeAnomalieRead[]>(`/commandes/${id}/anomalies`),

  create: (body: CommandeCreate) =>
    apiRequest<CommandeRead>("/commandes", { method: "POST", body }),

  submit: (id: number) => apiRequest<CommandeRead>(`/commandes/${id}/submit`, { method: "POST" }),
  ...
  delete: (id: number) => apiRequest<void>(`/commandes/${id}`, { method: "DELETE" }),
};
...
export const ventesApi = {
  list: (p?: VenteListParams) => apiRequest<VenteRead[]>(`/ventes${qs({ limit: 50, ...p })}`),

  count: (p?: Omit<VenteListParams, "skip" | "limit">) =>
    apiRequest<{ total: number }>(`/ventes/count${qs(p ?? {})}`),

  get: (id: number) => apiRequest<VenteRead>(`/ventes/${id}`),

  create: (body: VenteCreate) => apiRequest<VenteRead>("/ventes", { method: "POST", body }),

  // ── Proforma (devis) — cahier des charges §9.1-§9.3 ───────────────────────
  createProforma: (body: VenteProformaCreate) =>
    apiRequest<VenteRead>("/ventes/proforma", { method: "POST", body }),

  updateProforma: (id: number, body: VenteProformaUpdate) =>
    apiRequest<VenteRead>(`/ventes/${id}/proforma`, { method: "PATCH", body }),

  rejectProforma: (id: number, body: VenteProformaReject) =>
    apiRequest<VenteRead>(`/ventes/${id}/proforma/reject`, { method: "POST", body }),

  transform: (id: number, body: VenteTransformRequest) =>
    apiRequest<VenteRead>(`/ventes/${id}/transform`, { method: "POST", body }),

  void: (id: number) => apiRequest<VenteRead>(`/ventes/${id}/void`, { method: "POST" }),
  ...
  delete: (id: number) => apiRequest<void>(`/ventes/${id}`, { method: "DELETE" }),

  // ── Returns ──────────────────────────────────────────────────────────────
  getRetours: (id: number) => apiRequest<VenteRetourRead[]>(`/ventes/${id}/retours`),
...
export const creancesApi = {
  ...
  get: (id: number) => apiRequest<CreanceRead>(`/creances/${id}`),

  create: (body: CreanceCreate) => apiRequest<CreanceRead>("/creances", { method: "POST", body }),
  ...
  // §8.2 — relance (manuelle) + balance âgée
  sendRelance: (id: number, body: RelanceCreate) =>
    apiRequest<CreanceRead>(`/creances/${id}/relance`, { method: "POST", body }),

  agedBalance: (boutiqueId?: number) =>
    apiRequest<AgedBalanceBucket[]>(
      `/creances/reports/aged-balance${qs({ boutique_id: boutiqueId })}`,
    ),
};
...
export const expensesApi = {
  ...
  get: (id: number) => apiRequest<ExpenseRead>(`/expenses/${id}`),

  create: (body: ExpenseCreate) => apiRequest<ExpenseRead>("/expenses", { method: "POST", body }),

  update: (id: number, body: ExpenseUpdate) =>
    apiRequest<ExpenseRead>(`/expenses/${id}`, { method: "PATCH", body }),

  delete: (id: number) => apiRequest<void>(`/expenses/${id}`, { method: "DELETE" }),
...
export const cashApi = {
  ...
  // §10 — annulation/correction d'un encaissement
  cancelMovement: (id: number, body: CashMovementCancel) =>
    apiRequest<CashMovementRead>(`/cash/movements/${id}/cancel`, { method: "POST", body }),
};
...
export const usersApi = {
  ...
  get: (id: number) => apiRequest<UserRead>(`/users/${id}`),

  create: (body: UserCreate) => apiRequest<UserRead>("/users", { method: "POST", body }),

  update: (id: number, body: UserUpdate) =>
    apiRequest<UserRead>(`/users/${id}`, { method: "PATCH", body }),

  delete: (id: number) => apiRequest<void>(`/users/${id}`, { method: "DELETE" }),

  listGroups: (userId: number) => apiRequest<UserGroupRead[]>(`/users/${userId}/groups`),
  ...
  listStores: (userId: number) => apiRequest<StoreUserRead[]>(`/users/${userId}/stores`),
...
export const accessApi = {
  ...
  listGroupPermissions: (groupId?: number) =>
    apiRequest<GroupPermissionRead[]>(
      `/access/group-permissions${qs({ group_id: groupId, limit: 500 })}`,
    ),
...
export const notificationsApi = {
  ...
  markAllRead: () => apiRequest<{ updated: number }>("/notifications/read-all", { method: "POST" }),
};

// ─── System settings ──────────────────────────────────────────────────────────

export const settingsApi = {
  list: (group_name?: string) => apiRequest<SettingRead[]>(`/system/settings${qs({ group_name })}`),
...
export const qk = {
  ...
  transferts: {
    list: (p?: object) => ["transferts", p] as const,
    detail: (id: number) => ["transferts", id] as const,
  },
  activityLogs: {
    list: (p?: object) => ["activity-logs", p] as const,
  },
  users: {
...
export interface DashboardKpi {
  ...
  stock_alerte: number;
  creances_actives: number;
  creances_montant: number;
  encaissements_jour: number;
  remboursements_jour: number;
  total_decaissement: number;
  caisse_solde: number | null;
  caisse_ouverte: boolean;
  boutiques_total: number;
  boutiques_actives: number;
}
export interface DashboardVenteEvolution {
  date: string;
  count: number;
  ca: number;
}
...
export interface DashboardTopBoutique {
  boutique_id: number;
  nom: string;
  nb_ventes: number;
  ca: number;
}
export interface DashboardModePaiement {
  mode: string;
  label: string;
  montant: number;
  count: number;
...
export interface CollectionWeekPoint {
  week: string;
  collected: number;
  outstanding: number;
}
...
export interface SupplyStats {
  nb_commandes: number;
  delai_moyen_validation_jours: number | null;
  delai_moyen_livraison_jours: number | null;
  taux_rejet_pct: number;
  taux_ecart_pct: number;
}
export interface ReportsData {
  ...
  aged_balance: AgedBalanceBucket[];
  supply_stats: SupplyStats;
}

export const reportsApi = {
  getData: (boutique_id?: number) =>
    apiRequest<ReportsData>(`/reports/data${boutique_id ? `?boutique_id=${boutique_id}` : ""}`),
};

// ─── Journal d'activité (cahier des charges §14) ──────────────────────────────
...
export const activityLogsApi = {
  ...
  ) => apiRequest<ActivityLogRead[]>(`/system/activity-logs${qs({ skip: 0, limit: 200, ...p })}`),
};

// ─── Transferts inter-boutiques (cahier des charges §7.4/§12) ────────────────

export const transfertsApi = {
  list: (p?: ListParams & { boutique_id?: number; statut?: TransfertStatut }) =>
    apiRequest<TransfertRead[]>(`/transferts${qs({ skip: 0, limit: 200, ...p })}`),

  get: (id: number) => apiRequest<TransfertRead>(`/transferts/${id}`),

  create: (body: TransfertCreate) =>
    apiRequest<TransfertRead>("/transferts", { method: "POST", body }),

  receive: (id: number, body: TransfertReceive) =>
    apiRequest<TransfertRead>(`/transferts/${id}/receive`, { method: "POST", body }),

  cancel: (id: number, body: TransfertCancel) =>
    apiRequest<TransfertRead>(`/transferts/${id}/cancel`, { method: "POST", body }),
};
